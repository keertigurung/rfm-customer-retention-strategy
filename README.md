...
1. **Data Cleaning (Python):** Removed cancellations, invalid prices/quantities, null customers. Parsed dates, created revenue column.
2. **RFM Scoring:** Computed Recency, Frequency, Monetary per customer. Applied `pd.qcut()` with 5 bins (quintiles) after `rank(method='first')` to handle ties.
3. **Segmentation:** Priority-ordered decision tree (Recency → Frequency → Monetary). First match wins. 7 segments, default = Low Priority.
4. **Churn Definition:** `churned = 1` if recency ≥ 90 days.
5. **SQL Analysis (PostgreSQL):** 7 business queries answering revenue, churn, product, and trend questions.
6. **Dashboard (Power BI):** Two-page interactive dashboard (Overview + Churn & Risk).
...
|---|---|---|
| 1 | Champions | Loyalty tier / early access |
| 2 | Loyal Customers | Cross-sell complementary categories |
| 3 | Potential Loyalists | Bundle offer to increase frequency |
| 4 | Can't Lose Them | Personalised win-back with exclusive offer |
| 5 | At Risk | Automated day-60 "we miss you" nudge |
| 6 | Hibernating | Low-cost email with high-value product recommendation |
| 7 | Low Priority | Suppress from campaigns |

*Rows 1–3: Proactive | Rows 4–6: Reactive | Row 7: Exclusion*
